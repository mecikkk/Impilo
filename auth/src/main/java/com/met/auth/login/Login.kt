...
class Login : AppCompatActivity() {
        ...
        val signInObserver = Observer<Boolean> {
            if (it)
                ViewUtils.createSnackbar(
                    bg,
                    bg.resources.getString(com.met.impilo.R.string.success_login)
                ).show()
            else
                ViewUtils.createSnackbar(
                    bg,
                    bg.resources.getString(com.met.impilo.R.string.failed_signing_in)
                ).show()
        }

        connectionInformation.isConnected.observe(this, connectionObserver)
        vModel.signInSuccessful.observe(this, signInObserver)
    }
    ...
    private fun init() {
        // Tworzenie BroadcastReceiver'a z informacją o połączeniu z inernetem
    ...
    private fun createNetworkDialog() : Dialog {
        var networkDialog = ViewUtils.createInternetConnectionDialog(this)
    ...
    private fun validate(): Boolean {
        val invalidEmail = (Utils.isEditTextEmpty(
            email_textfield,
            email_input_layout,
            resources.getString(com.met.impilo.R.string.field_required)
        ) ||
                !Utils.isEmailValid(
                    email_textfield.text.toString(),
                    email_input_layout,
                    resources.getString(com.met.impilo.R.string.invalid_email_format)
                ))

        val invalidPassword = (Utils.isEditTextEmpty(
            password_textfield,
            password_input_layout,
            resources.getString(com.met.impilo.R.string.field_required)
        ) ||
                !Utils.isPasswordLengthValid(
                    password_textfield.length(),
                    password_input_layout,
                    resources.getString(com.met.impilo.R.string.password_length_error)
                ))
    ...
    private fun startAnimation() {
        ...
        app_name_textview.animate().translationY(impiloDefaultPosition[1].toFloat())
            .setStartDelay(2000).setDuration(500)
            .setInterpolator(LinearOutSlowInInterpolator()).start()
        bg.animate().alpha(0.5f).setStartDelay(2000).setDuration(500)
            .setInterpolator(LinearOutSlowInInterpolator()).start()
        ...
        if (login_fields_layout.height / ScreenUtils.getScreenHeight(this) > 0.75) {
            app_name_textview.animate().alpha(0f).setStartDelay(2100).setDuration(500)
                .setInterpolator(LinearOutSlowInInterpolator()).start()
        }
    }
