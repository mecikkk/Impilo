...
object ViewUtils {
    ...
    fun getDatePicker(view: View?, startFromYear: Boolean, selectDate: Date, dateResult: (Date) -> Unit): DatePickerDialog {
        val calendar = Calendar.getInstance()

        calendar.set(Calendar.YEAR, selectDate.getIntYear())
        calendar.set(Calendar.MONTH, selectDate.getIntMonth()-1)
        calendar.set(Calendar.DAY_OF_MONTH, selectDate.getIntDay())
        ...
        val date: DatePickerDialog.OnDateSetListener = DatePickerDialog.OnDateSetListener { _, year, month, dayOfMonth ->
            ...
            dateResult(calendar.time)
        }

        val datePickerDialog = DatePickerDialog(view!!.context, com.met.impilo.R.style.DatePickerTheme, date, calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH),
    ...
    fun createSnackbar(parent: View, message: String): Snackbar {
        val snackbar: Snackbar = Snackbar.make(parent, message, Snackbar.LENGTH_LONG).apply {
    ...
    fun createInternetConnectionDialog(context: Context): Dialog {
        val internetConnectionDialog = Dialog(context, android.R.style.Theme_Material_Dialog_NoActionBar)
    ...
    fun isLightModeOn(context: Context): Boolean = when (context.resources.configuration.uiMode and Configuration.UI_MODE_NIGHT_MASK) {
